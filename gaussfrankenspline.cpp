// Code generated by Stan version 2.4
...
sq_distance(const T0__& x,
    ...
    typedef typename boost::math::tools::promote_args<T0__, T1__>::type return_t__;
...
(void) propto__;
    {
        ...
        Eigen::Matrix<return_t__,Eigen::Dynamic,1>  xy(2);
        (void) xy;   // dummy to suppress unused var warning
        stan::math::initialize(d, std::numeric_limits<double>::quiet_NaN());
        stan::math::initialize(xy, std::numeric_limits<double>::quiet_NaN());
        if (as_bool(logical_gt(x,y))) {
            stan::math::assign(get_base1_lhs(xy,1,"xy",1), (x - y));
            stan::math::assign(get_base1_lhs(xy,2,"xy",1), (((2.0 * pi()) - x) + y));
            stan::math::assign(d, min(xy));
        } else {
            stan::math::assign(get_base1_lhs(xy,1,"xy",1), (y - x));
            stan::math::assign(get_base1_lhs(xy,2,"xy",1), (((2.0 * pi()) - y) + x));
            stan::math::assign(d, min(xy));
        }
        return pow(d,2);
    }
...
gp_generalized_sq_exp(const T0__& eta_sq,
    ...
    typedef typename boost::math::tools::promote_args<T0__, T1__, T2__, T3__>::type return_t__;
    const static bool propto__ = true;
(void) propto__;
    {
        Eigen::Matrix<return_t__,Eigen::Dynamic,Eigen::Dynamic>  Sigma(get_base1(dims(x),1,"dims(x)",1),get_base1(dims(x),1,"dims(x)",1));
...
make_knots(const int& n_knots,
    ...
    typedef typename boost::math::tools::promote_args<T1__, T2__>::type return_t__;
    const static bool propto__ = true;
(void) propto__;
    {
        Eigen::Matrix<return_t__,Eigen::Dynamic,1>  knots(n_knots);
        (void) knots;   // dummy to suppress unused var warning
        return_t__ interknot_length;
...
(void) propto__;
    {
        ...
        stan::math::assign(J, get_base1(dims(knot_points),1,"dims(knot_points)",1));
        for (int j = 1; j <= J; ++j) {
            stan::math::assign(y, (y + (((get_base1(knot_weights,j,"knot_weights",1) * 1) / (knot_scale * pow((2.0 * pi()),0.5))) * ((exp((-(pow((pow(sq_circ_distance(theta,get_base1(knot_points,j,"knot_points",1), pstream__),0.5) + (2.0 * pi())),2)) / (2.0 * pow(knot_scale,2)))) + exp((-(pow(pow(sq_circ_distance(theta,get_base1(knot_points,j,"knot_points",1), pstream__),0.5),2)) / (2.0 * pow(knot_scale,2))))) + exp((-(pow((pow(sq_circ_distance(theta,get_base1(knot_points,j,"knot_points",1), pstream__),0.5) + (2.0 * pi())),2)) / (2.0 * pow(knot_scale,2))))))));
        }
        return y;
    }
}
...
day_to_angle(const T0__& day, std::ostream* pstream__) {
    typedef typename boost::math::tools::promote_args<T0__>::type return_t__;
    const static bool propto__ = true;
(void) propto__;
    {
        return_t__ angle;
...
public:
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        // model body
        {
            ...
            for (int i = 1; i <= n_knots; ++i) {
                stan::math::assign(get_base1_lhs(mu,i,"mu",1), theta_mu);
            }
            lp_accum__.add(multi_normal_log<propto__>(knot_weights, mu, theta_Sigma));
            for (int i = 1; i <= n_points; ++i) {
                stan::math::assign(get_base1_lhs(positions_mu,i,"positions_mu",1), yday_circular_spline(get_base1(day_of_year,i,"day_of_year",1),knot_points,knot_weights,knot_scale, pstream__));
            }
            lp_accum__.add(normal_log<propto__>(positions, positions_mu, 1));
        }
